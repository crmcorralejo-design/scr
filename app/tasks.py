"""
BookingScraper/app/tasks.py  v3.0  [FIX NO-OVERWRITE + images_count]
Tareas Celery para scraping asíncrono - BookingScraper Pro
Windows 11 + Python 3.14.3

CAMBIOS v3.0 [FIX INTEGRIDAD DE DATOS - NO SOBRESCRIBIR]:

  DIAGNÓSTICO v2.2 → v3.0:
  ──────────────────────────────────────────────────────────────
  ● El INSERT de hotels usaba ON CONFLICT (url_id, language) DO UPDATE SET
    que sobrescribía todos los campos de un hotel ya scraped si se ejecutaba
    de nuevo. Esto violaba el requisito "No sobrescribir datos" (req. 6d).
    RIESGO: Si el orden de procesamiento cambiaba (por ejemplo, un idioma
    incorrecto llegaba antes que el inglés), el DO UPDATE sobreescribía datos
    correctos con datos incorrectos del rescraping.

  [FIX #38] Hotel INSERT: ON CONFLICT DO UPDATE → ON CONFLICT DO NOTHING
    ANTES v2.2: ON CONFLICT (url_id, language) DO UPDATE SET (todos los campos)
    AHORA v3.0: ON CONFLICT (url_id, language) DO NOTHING
    EFECTO: Si ya existe un registro hotel para (url_id, language), se respeta
    el dato existente. La tarea Celery logea el evento como 'skipped_existing'
    para trazabilidad. Para forzar rescraping, el operador elimina el registro.
    COHERENTE CON: scraper_service.py v5.1 que aplica la misma política.

  [FIX #39] images_count en DO UPDATE eliminado (ya no aplica con DO NOTHING).
    Consistencia: images_count se gestiona únicamente en el UPDATE posterior
    a la descarga de imágenes, nunca en el INSERT/UPDATE de datos de hotel.
...
def scrape_hotel_task(self, url_id: int):
    ...
    DEFECTOS CORREGIDOS respecto a v2.2:
      ...
      [FIX-E v3.0] Hotel INSERT: ON CONFLICT DO NOTHING en lugar de DO UPDATE SET.
              El código anterior sobrescribía datos existentes, violando req. 6d.
    ...
    try:
        ...
        for lang in languages:
            try:
                ...
                # [FIX-E v3.0] ON CONFLICT DO NOTHING: preserva datos existentes.
                # Si el registro ya existe para (url_id, lang), se omite el INSERT
                # y se logea para trazabilidad. No se sobrescriben datos previos.
                # Coherente con scraper_service.py v5.1 (_save_hotel DO NOTHING).
                ...
                if insert_result.rowcount == 0:
                    # Registro ya existía — ON CONFLICT DO NOTHING → preservado
                    logger.debug(
                        f"  ℹ️ [{url_id}][{lang}] Registro ya existe en BD — "
                ...
                # AHORA v3.0: siempre lang == 'en' → siempre carpeta hotel_{id}/en/
